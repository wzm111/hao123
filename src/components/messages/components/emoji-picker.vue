...
const requireEmoji = import.meta.globEager('../emoji/*.svg');
const files = {};
Object.keys(requireEmoji).forEach((fileName) => {
    const name = fileName.replace(/\.\.\/emoji\/|\.svg/g, '');
    files[name] = requireEmoji[fileName].default;
});
